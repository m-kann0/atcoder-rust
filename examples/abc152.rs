...
fn solve(input: &str) -> String {
    ...
    let mut merged_factors: HashMap<usize, usize> = HashMap::new();
    for i in 0..n {
        let f = prime_factorization(a[i]);
        for (&k, &v) in f.iter() {
            if !merged_factors.contains_key(&k) {
                merged_factors.insert(k, v);
            } else {
                let new_value = max(*merged_factors.get(&k).unwrap(), v);
                merged_factors.insert(k, new_value);
            }
        }
        factors.push(f);
    }
