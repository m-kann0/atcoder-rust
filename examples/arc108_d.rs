...
fn solve(input: &str) -> String {
    ...
    let caa: char = iterator.next().unwrap().parse().unwrap();
    let cab: char = iterator.next().unwrap().parse().unwrap();
    let cba: char = iterator.next().unwrap().parse().unwrap();
    let cbb: char = iterator.next().unwrap().parse().unwrap();
    ...
    if cab == 'A' {
        ...
        if cba == 'A' {
            return f1(n).to_string();
        }
        return f2(n).to_string();
    }
...
fn f1(n: usize) -> usize {
    let mut memo = vec![ModInt1000000007::new(1); n + 1];
    for i in 4..=n {
