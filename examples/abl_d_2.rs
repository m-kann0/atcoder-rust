...
fn solve(input: &str) -> String {
    ...
    const MAX_A: usize = 300_005;
    let mut segtree = Segtree::<Max<usize>>::new(MAX_A);
    for _ in 0..n {
        let ai: usize = iterator.next().unwrap().parse().unwrap();
        let l: isize = ai as isize - k as isize;
        let r: usize = ai + k + 1;
        let l = max(0, l) as usize;
        let r = min(r, MAX_A);
        segtree.set(ai as usize, segtree.prod(l, r) + 1);
    }

    let ans = segtree.prod(0, MAX_A);
    ans.to_string()
}
